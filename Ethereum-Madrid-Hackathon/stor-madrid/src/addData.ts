...
const main = async () => {
    ...
    for (let i = 0; i < userArray.length; i++) {
        await User.CREATE_USER_RAW(userArray[i], blockchainProperties)
        console.log(userArray[i].firstName + ' ' + userArray[i].surname + ' (' + userArray[i].organization + ') onboarded')
    }


    const assetProps_Engie1: ProducingAssetProperties = {
    ...
    const assetProps_Engie2: ProducingAssetProperties = {
        ...
        assetType: AssetType.RunRiverHydro,
        certificatesCreatedForWh: 0,
        lastSmartMeterCO2OffsetRead: 0,
        cO2UsedForCertificate: 0,
        complianceRegistry: Compliance.none,
    ...
    const assetProps_Engie3: ProducingAssetProperties = {
    ...
    const assetProps_Engie4: ProducingAssetProperties = {
    ...
    const assetProps_Kathrin: ProducingAssetProperties = {
    ...
    const assetProps_Steffen: ProducingAssetProperties = {
    ...
    const assetProps_TWL1: ProducingAssetProperties = {
    ...
    const assetProps_TWL2: ProducingAssetProperties = {
    ...
    const assetProps_TWL3: ProducingAssetProperties = {
    ...
    const assetProps_TWL4: ProducingAssetProperties = {
    ...
    const assetProps_TWL5: ProducingAssetProperties = {
    ...
    const assetProps_SP1: ProducingAssetProperties = {
    ...
    const assetProps_SP2: ProducingAssetProperties = {
    ...
    const assetProps_SP3: ProducingAssetProperties = {
    ...
    const assetProps_SP4: ProducingAssetProperties = {
    ...
    const assetProps_SP5: ProducingAssetProperties = {
    ...
    const assetProps_sonnen1: ProducingAssetProperties = {
    ...
    const assetProps_sonnen2: ProducingAssetProperties = {
    ...
    for (let i = 0; i < assetProducingArray.length; i++) {
        ...
        let foundAsset = allAssets.find((passet: ProducingAsset) => (passet.smartMeter === assetProducingArray[i].smartMeter))

        if (!foundAsset) {
            let asset = await ProducingAsset.CREATE_ASSET_RAW(assetProducingArray[i], blockchainProperties)
    ...
    let allConsumingAssets = await ConsumingAsset.GET_ALL_ASSETS(blockchainProperties)
    ...
    for (let i = 0; i < assetConsumingArray.length; i++) {
        ...
        let foundAsset = allConsumingAssets.find((casset: ConsumingAsset) => (casset.smartMeter === assetConsumingArray[i].smartMeter))

        if (!foundAsset) {
            let asset = await ConsumingAsset.CREATE_ASSET_RAW(assetConsumingArray[i], blockchainProperties)
    ...
    const allDemands = await Demand.GET_ALL_ACTIVE_DEMANDS(blockchainProperties)

    for (let i = 0; i < demandArray.length; i++) {


        let foundDemand = allDemands.find((d: Demand) => (
            d.buyer.toLowerCase() === demandArray[i].buyer.toLowerCase()
        ...
        if (!foundDemand) {
            ...
            let demand = await Demand.CREATE_DEMAND_RAW(demandArray[i], blockchainProperties, "0xd173313a51f8fc37bcf67569b463abd89d81844f")
